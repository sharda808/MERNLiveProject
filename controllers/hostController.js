...
exports.postAddHome =(req,res,next) => {
  ...
  if(!req.file){
    return res.status(400).send('No valid image provieded');
  }
  ...
  existingHome.houseName = houseName;
   existingHome.price= price;
    existingHome.location = location;
     ...
     if(req.file){
      deleteFile(existingHome.photoUrl.replace(/^\//, ""));
      existingHome.photoUrl ="/" + req.file.path.replace(/^\//, "");
     }
...
res.render('host/host-homes',{homes:registeredHomes,pageTitle:'Host Homes',isLoggedIn:req.session.isLoggedIn, user:req.session.user
